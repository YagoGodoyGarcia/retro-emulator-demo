...
function normalizeGame(raw, source) {
  const resolvedSource = raw.source || source;
  return {
    gameId: raw.gameId,
    title: raw.title,
    core: raw.core,
    ...
    genre: raw.genre || "",
    tags: raw.tags || [],
    // Jogo cadastrado pelo admin (upload direto, fora do catálogo estático
    // curado) sempre aparece como "Exclusivo"/destaque, primeiro na home —
    // não depende do checkbox "featured" ter sido marcado.
    featured: resolvedSource === "dynamic" ? true : Boolean(raw.featured),
...
// O que a vitrine, /api/keychains e /play/:id mostram — jogo em "review"
// (importado em lote, ainda sem capa/revisão) fica de fora até alguém
// publicar. Jogo antigo sempre bate aqui, já que normalizeGame() default
// pra "published" quando o campo não existe.
