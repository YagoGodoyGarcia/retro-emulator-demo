...
async function upload(kind, filename, buffer, contentType) {
  if (durable) {
    ...
    const blob = await put(`${kind}/${filename}`, buffer, {
      ...
      // O nome é sempre `${gameId}${ext}` (game-entry.js) — determinístico e
      // único por jogo. Editar a capa/ROM de um jogo já existente sobe pro
      // MESMO caminho de propósito (é a substituição, não um upload novo);
      // sem isso o SDK recusa com "blob already exists" (bug real visto em
      // produção). allowOverwrite não abre brecha pra sobrescrever o arquivo
      // de OUTRO jogo — o nome já é escopado ao próprio gameId.
      allowOverwrite: true,
      token: blobToken,
    });
    return blob.url;
